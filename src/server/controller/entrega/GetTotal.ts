...
export const getTotalEntrega = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    // Chama a função 'total' que calcula o total das colunas do caixa
    const totalCaixa = await EntregadorProvider.getTotal();
    ...
    res.status(StatusCodes.OK).json({
      ...
      totalQuantidaEntrega: totalCaixa.totalQuantidaEntrega,
      totalValorEntregaDinheiro: totalCaixa.totalValorEntregaDinheiro,
      totalValorEntregaPix: totalCaixa.totalValorEntregaPix,
    });
    return;
  } catch (error) {
