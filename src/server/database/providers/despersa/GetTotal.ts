...
> => {
  try {
    const result = await Knex(ETableNames.DESPERSA)
      .sum({
        totalValorDespersa: "valorDespersa",
      })
      .first();

    if (!result) {
      throw new Error("Nenhum registro encontrado para cálculo do total.");
    }

    const totalValorDespersaRaw = Number(result.totalValorDespersa);
    const totalValorDespersa = isNaN(totalValorDespersaRaw)
      ? 0
      : totalValorDespersaRaw;

    return { totalValorDespersa };
  } catch (error) {
    console.error("Erro ao calcular o total:", error); // Log mais detalhado no servidor
    throw new Error(
      `Erro no servidor: ${
