...
> => {
  try {
    ...
    const totalQuantidaEntrega = Number(result.totalQuantidaEntrega) || 0;
    const totalValorEntregaDinheiro =
      Number(result.totalValorEntregaDinheiro) || 0;
    const totalValorEntregaPix = Number(result.totalValorEntregaPix) || 0;
